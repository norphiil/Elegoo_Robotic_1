...
public:
    ...
    void init(int speed, float targetDistance, float timePerCell, uint8_t wallDistanceThreshold);
    ...
    void straightLine(uint8_t cells, bool assisted);
    void straightUnassisted(uint8_t cells);
    bool straightAssisted(uint8_t cells, unsigned long *lastTime, uint16_t distance);
...
private:
    ...
    int speed;
    float targetDistance, timePerCell;
